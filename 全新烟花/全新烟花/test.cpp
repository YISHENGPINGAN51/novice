...
{
	//音乐 爱的翅膀
	mciSendString(L"open ./fire/unraval.mp3 alias music", 0, 0, 0);	//send(发送)	string（字符串）
	...
	for (int i = 0; i < 50; i++)
	{
		//清除屏幕
		cleardevice();
		int x = 550 + 180 * sin(2 * PI / 60 * i);
		int y = 200 + 180 * cos(2 * PI / 60 * i);
		//设置字体样式
		settextstyle(i, 0, L"楷体");
		settextcolor(RGB(0, 202, 0));
		setbkmode(TRANSPARENT);
		outtextxy(x, y, L"祝新年快乐");
		Sleep(20);
	}
	_getch();//按任意键继续
	cleardevice();
	settextstyle(25, 0, L"楷体");
	outtextxy(300, 100, L"牛年运程零时的钟声响彻天涯，");
	outtextxy(300, 150, L"新年的列车准时出发。");
	outtextxy(300, 200, L"它驼去一个难忘的岁月，");
	outtextxy(300, 250, L"迎来了又一度火红的年华。");
	outtextxy(300, 300, L"祝新年快乐！");
	outtextxy(500, 400, L"From: 房蕴力");
	outtextxy(500, 450, L"2021年2月11日");
	Sleep(3000);
}
